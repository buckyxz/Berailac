...
const PRODUCTS: Product[] = [
    {
        ...
        description: `
            ...
            Chondroitin sulfate bağ dokusuna daha fazla besin geçmesine, suyun kıkırdaktan geçmesine ve bağ dokusunun sağlamlığını,
            esnekliğini arttırarak onarıma yardımcı olabilir.
...
const ProductPage = ({ params }: { params: { slug: string } }) => {
    ...
    const [product, setProduct] = useState<Product | undefined>(
        PRODUCTS.find((producteach) => producteach.href === params.slug)
    );
    ...
    }, [product, router]); // Bağımlılıkları ekledik
        ...
        <div className="w-full h-screen flex overflow-hidden justify-center items-center">
            <div className="w-full h-full flex mx-10 lg:mx-48 gap-36">
                ...
                <div className="w-full h-full justify-center items-center hidden lg:flex">
                    {product?.src && (
                        <Image src={product.src} alt={product.name} width={500} height={500} className="w-full rounded-2xl" />
                    )}
                </div>
            </div>
        </div>
